...
def selectAlpha_j(svm, alpha_i, error_i):
    ...
    candidateAlphaList =  (svm.errorCache[:, 0].A)[0]  # mat.A return array
    maxStep = 0;
    alpha_j = 0;
    error_j = 0
...
def testSVM(svm, test_x):
    test_x = mat(test_x)
    numTestSamples = test_x.shape[0]
    predict = np.ones((numTestSamples, 1), dtype='float32')
    supportVectorsIndex = nonzero(svm.alphas.A > 0)[0]
    supportVectors = svm.train_x[supportVectorsIndex]
    supportVectorLabels = svm.train_y[supportVectorsIndex]
    supportVectorAlphas = svm.alphas[supportVectorsIndex]
    for i in xrange(numTestSamples):
        kernelValue = calcKernelValue(supportVectors, test_x[i, :], svm.kernelOpt)
        predict[i] = kernelValue.T * multiply(supportVectorLabels, supportVectorAlphas) + svm.b
    return predict
